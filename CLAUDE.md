...
- `%TEMP%\doomslop-pages.mjs` — 6 assertions on the **production build under a path prefix**, which is the only way to catch a wrong Vite `base`. It runs its own static server that serves `dist/` at `/DoomSlop/` and logs a status per request, so a 404 on an asset is reported rather than just showing as a blank screen. Everything is asserted through the DOM, since Vite strips `window.__game` from a production build — reading `HP 100` out of `#health` is the proxy for "the bundle booted and reached the frame loop". Run it after touching `vite.config.js`, `index.html`, or anything about the deploy. Note that neither `npm run dev` nor `npm run preview` can catch this class of bug: both serve from the root, where a base of `/` works.
