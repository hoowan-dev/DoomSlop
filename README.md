...
A minimal browser FPS built on [three.js](https://threejs.org/): enemies float toward you, you shoot them. Fifty kills summons a boss; kill it and the next round starts.
...
Two things about the boss are worth knowing. It is **not** consumed when it reaches you — it has to be shot down, so it parks on you and hits for 20 HP every 1.2 seconds instead of dying on contact. It's also slower than a floater, so you can always back away from it. And only *shot* kills count toward the 50 — a floater that reaches you and vanishes costs you 25 HP and leaves the `KILLS` count where it was, the same rule the score already follows. The count hides itself during the boss fight, when there's nothing left to tally.
...
All balance numbers live in [src/config.js](src/config.js) — that's the file to edit if it's too easy or too hard. Effect tuning (tracer lifetime, spark counts, colors, gravity) is in the `EFFECTS` block there; pitches, durations and volume are in `SOUND`, where `masterVolume` turns everything down at once; the radar's size, range and colors are in `MINIMAP`; the score popups' lifetime, rise and pool size are in `POPUP` (their green is in `style.css`). The boss's own stats are in `BOSS` (it has every field `ENEMY` does, plus `attackInterval`), and `ROUNDS.killsPerRound` is the 50 — drop it to 5 if you want to see the boss without grinding for it.

## Not implemented

Weapon variety, a persistent high score, and mobile/touch controls. There are two enemy tiers (floater and boss) and no more planned. Rounds don't get harder either — boss health and the 50-kill quota are the same every round, so the only escalation is the floater spawn ramp. The arena is also a bare box — no cover, no level geometry.
