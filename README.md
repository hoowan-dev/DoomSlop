...
| [src/portals.js](src/portals.js) | The two wall portals: placement and the traversal test |
...
Two sections of the walls are portals, marked by a glowing cyan square standing on the floor with a haze of blue spilling onto the wall and floor around it. Walk into one and you come out of the other, stepping into the arena off that wall with the heading you walked in with — so a portal in the wall behind you is a shortcut across the map, and the one chasing you has to take the long way. They're placed on two different walls at the start of every round, including after a retry and after a `~` skip, so the pair of shortcuts you learn lasts exactly one round. Bullets pass straight through the square, and so does a jump: taking one mid-hop keeps your arc.
