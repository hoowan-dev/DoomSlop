...
| [assets/images/](assets/images/) | The five boss portraits |
...
Everything dangerous or worth having burns: each enemy carries a halo around its body and throws a pool of colored light on the floor under it — red for a floater, purple for the boss — and so does every drop, in its own color. Shooting throws a brief flash of light off the muzzle. The lights land on the arena and the drops only, never on another enemy: a crowd of floaters would otherwise wash each other flat and stop being countable, which is the one thing the glow exists to help with. The twelve nearest enemies get one, since the pool is fixed and a distant enemy holding a light while one in your face has none is the only arrangement you'd notice.
...
Five sound effects play: firing, killing an enemy, taking damage, a swelling choir when you pick something up, and a click when you pause or resume (rising to resume, falling to pause). They're synthesized at runtime from oscillators and a noise buffer — no audio files involved. Audio starts on the click that locks the mouse, since browsers only allow it from a user gesture.
