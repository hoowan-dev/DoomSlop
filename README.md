...
| [src/portals.js](src/portals.js) | The two pairs of wall portals: placement, the traversal test, and the view through each |
...
Four sections of the walls are portals — **two pairs, one blue and one orange**, each a glowing doorway with a haze of its own color spilling onto the wall and floor around it, and inside each square you can see the far side of the arena. Walk into one and you come out of its *partner*, stepping into the arena off that wall with the heading you walked in with — so a portal in the wall behind you is a shortcut across the map, and the one chasing you has to take the long way. The color is how you know where a doorway goes: blue leads to blue and orange to orange, and since the pairs never mix there's no guessing which of three other squares you're about to be thrown at.

There is exactly one doorway in each of the four walls, and a pair's two mouths are always on *facing* walls — so one pair owns the two walls at either end of one axis and the other pair owns the two at either end of the other. That makes a trip a straight line across the box: the doorway you're looking at leads to the wall directly behind you, and you come out of it still facing the way you were. It's a stronger promise than a pair simply being somewhere near each other — you never have to wonder which square you're about to be thrown at, because it's the one you had your back to, and the wall you're facing only ever carries the one. The trade is that no trip turns a corner any more; the arena wraps rather than folds. The whole lot is re-placed at the start of every round, including after a retry and after a `~` skip, so the four shortcuts you learn last exactly one round. Bullets pass straight through a square, and so does a jump: taking one mid-hop keeps your arc. Stepping through plays a rising sci-fi whoosh — a pair of detuned tones climbing three octaves over a rush of swept noise.
...
The arena's four walls are drawn on it too, as a faint grey box you move around inside, with each portal a short segment set into the wall it's cut from, in its own pair's color — the same blue or orange the doorway glows. That box is the only fixed thing on the radar, and it's what makes the circle a *position* rather than a window: being backed into a corner, and which way the nearest portal is, are now things you can read at a glance instead of having to turn around and look. The colors carry the same information here as they do in the room, and it's arguably more useful on the radar: two blue segments on the map are the two ends of one shortcut, so you can see where a doorway comes out without having walked through it.
...
All balance numbers live in [src/config.js](src/config.js) — that's the file to edit if it's too easy or too hard. Effect tuning (tracer lifetime, spark counts, colors, gravity) is in the `EFFECTS` block there; pitches, durations and volume are in `SOUND`, where `masterVolume` turns everything down at once; the radar's size, range and colors are in `MINIMAP`, including `wallColor`/`wallThickness` for the arena box (dim on purpose — it's the backdrop, not a readout) and `portalBlue`/`portalOrange` with one shared `portalThickness` for the doorway segments, which are worth making *heavier* rather than longer if they're hard to spot, since a segment's ends are where the doorway's edges actually are; the score popups' lifetime, rise and pool size are in `POPUP` (their green is in `style.css`). `ENEMY.spawnHeightMin`/`spawnHeightMax` are the height band — set them both to `hoverHeight` for the old flat behaviour, or widen them for more vertical chaos, keeping both ends clear of the floor and the walls by the enemy radius. The boss's own stats are in `BOSS` (it has every field `ENEMY` does, plus `attackInterval` and `portraitScale`, the fraction of the orb its face covers; its height band is deliberately pinned to its hover height, so its entrance stays staged), and `ROUNDS.killsPerRound` is the kill quota — drop it to 5 if you want to see the boss without grinding for it. `ROUNDS.speedStep` is the 0.5 added per round, covering enemy speed and spawn rate together; set it to 0 for a flat difficulty curve, or 0.15 for a gentler climb. The drops are in `PICKUP`: `dropChance` is whether anything falls and `armorShare`/`bootsShare` split what it is (health is whatever's left over), with each cube's colors, glow and — for the boots — `duration` and `boost` on its own sub-block. `PLAYER.maxArmor` is the size of the armor pool, and `PLAYER.strafeRoll`/`strafeRollSpeed` are the strafe lean's angle and how fast it eases; a few degrees is a lean and eight is a list, so that one wants keeping small. The portals are in `PORTAL`: `size` is the doorway, `depth` how close counts as stepping through, `exitOffset` how far into the room you arrive — that last one **has to stay bigger than `depth`**, or you arrive inside the exit's own trigger and bounce between a pair — plus `margin`, which keeps them clear of the corners so a doorway's glow isn't cut in half by the wall beside it — turn it down and mouths can sit nearer the corners, up and they're pulled toward the middle of their walls. The two colors are `PORTAL.blue` and `PORTAL.orange`, each a `glow` block shaped like every other one in the file; everything physical about a doorway is shared between them, since the pairs differ only in color and which two walls they're in. `PORTAL.view` is the picture inside the square: `resolution` is the fraction of the screen each preview is rendered at and is the **one to turn down** if the game feels heavy, since it's paid for per screen pixel of every doorway you can see at the time — and with one doorway per wall that's never more than two of the four, because a 102° field of view can't take in more than two walls; `tint` is how much cyan is washed over the view; `brightness` is how far the far room sits above the fog, and it's the number to raise if a doorway reads as solid wall. The teleport sound is `SOUND.teleport` — `pitchFrom`/`pitchTo` are the climb, `detune` is how far the two tones sit apart, and the `noise*` fields are the rush of air under them. `EFFECTS.glowPool` is how many enemies can be lit at once — the number to turn *down* if a machine struggles, since the lights are paid for per screen pixel.
